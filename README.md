...
드론 이미지에서 **5개 부품**(암, 본체, 카메라, 랜딩기어, 프로펠러)을 자동으로 감지하고 분할하는 **세그멘테이션 시스템**입니다.
...
```
1. 드론 검사 담당자가 드론 사진 촬영
         ↓
2️. 웹 브라우저에서 이미지 업로드
         ↓
3. AI가 자동으로 부품 인식 및 분석
         ↓
4. 검출 결과 및 이상 여부 리포트 확인
         ↓
5. PDF 보고서 다운로드
```
...
Epoch 35 →  mIoU: 62.9%  |  Loss: 0.247  |  Acc: 90.0% 
...
프로젝트의 각 부분에 대한 상세한 설명은 별도 문서를 참조
...
- 이미지 업로드 → 실시간 분석 → 결과 확인
 ![드론분석메인페이지](docs/드론분석메인페이지.png)

#### 2️. 부품 검출 결과
- 5개 부품별 검출 상태 ( 정상 / 경고 /  미검출)
- 신뢰도 점수 시각화
![드론분석페이지](docs/드론분석결과.png)

#### 3️. 이미지 시각화
- 원본 이미지 /오버레이
![오버레이](docs/드론분석오버레이.png)

#### 4️. 모델 평가 대시보드
- 학습 곡선 (Accuracy, Loss)
- 클래스별 성능 차트
- 혼동 행렬 (Confusion Matrix)
![모델평가페이지](docs/모델평가1.png)
![](docs/모델평가2.png)
![](docs/모델평가3.png)
### 사용 흐름
